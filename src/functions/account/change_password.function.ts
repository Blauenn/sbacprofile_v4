...
export const account_password_update = async (
  ...
  if (settingsPassword_schema.safeParse(settingsPasswordObject).success) {
    ...
    const currentPasswordCheckJSON = JSON.stringify(currentPasswordCheckObject);
		...
		try {
			const accessTokenResponse: AccessTokenResponse = await axios.post(`${API_ENDPOINT}/api/v1/auth/login`, currentPasswordCheckJSON);
            ...
            password: settingsPasswordObject.new_password,
					...
					try {
						const changePasswordResponse: ChangePasswordResponse = await axios.post(`${API_ENDPOINT}/api/v1/auth/changePassword`, updatePasswordJSON, {headers: { "Content-Type": "application/json", authorization: accessTokenResponse.data.result.accessToken }})
