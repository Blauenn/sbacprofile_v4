...
export const classroom_update = async (
  ...
  if (validation) {
    ...
    // Update the classroom. //
    try {
			const response: ExplicitAxiosResponseInterface = await axios.post(`${API_ENDPOINT}/api/v1/classroom/update`, classroomUpdateJSON, {headers: { "Content-Type": "application/json" }});
