...
export const teacher_delete = async (teacher_ID: number) => {
  ...
  try {
		const response: ExplicitAxiosResponseInterface = await axios.post(`${API_ENDPOINT}/api/v1/teacher/delete`, teacherDeleteJSON, {headers: { "Content-Type": "application/json" }})
