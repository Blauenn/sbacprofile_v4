...
export const teacher_update = async (
  ...
  if (validation) {
    ...
    try {
			const response: ExplicitAxiosResponseInterface = await axios.post(`${API_ENDPOINT}/api/v1/teacher/update`, teacherUpdateJSON, {headers: { "Content-Type": "application/json" }})
