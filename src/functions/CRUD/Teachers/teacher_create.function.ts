...
export const teacher_create = async (
  ...
  if (validation) {
    ...
    try {
			const response: ExplicitAxiosResponseInterface = await axios.post(`${API_ENDPOINT}/api/v1/teacher/create`, teacherCreateJSON, {headers: { "Content-Type": "application/json" }})
