...
export const student_update = async (
  ...
  if (validation) {
    ...
    try {
			const response: ExplicitAxiosResponseInterface = await axios.post(`${API_ENDPOINT}/api/v1/student/update`, studentUpdateJSON , {headers: { "Content-Type": "application/json" }})
