...
export const student_create = async (
  ...
  if (validation) {
    ...
    try {
			const response: ExplicitAxiosResponseInterface = await axios.post(`${API_ENDPOINT}/api/v1/student/create`, studentCreateJSON, {headers: { "Content-Type": "application/json" }} )
